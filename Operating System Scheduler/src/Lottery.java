...
public class Lottery {
	...
	public Lottery(int quantum, int cost) {	
			...
			while(isdone(processCopy) == false) {				
				...
				for(int i = 0; i < n; i++) {
					if(processCopy.get(i).getStartTime() != 0) {
						sumOfPriority = sumOfPriority + processCopy.get(i).getPriority();
					}
					else {					
						processCopy.get(i).setPriority(0);
					}
				}
				
				//System.out.println("sumOfPriority: " + sumOfPriority);				
				int ticket = rand.nextInt(sumOfPriority)+1;
				...
				for(int i = 0; i < processCopy.size(); i++) {
					ticket = ticket - processCopy.get(i).getPriority();
					if(ticket <= 0) {
